...
namespace $rootnamespace$.Controllers
{
    ...
    public class CustomDatabaseController : Controller
    {
        ...
        private ActionResult DeleteHandler(IFileHandler handler)
        {
        ...
        private ActionResult PostHandler(IFileHandler handler)
        {
            // Get the posted file with meta data from the request
            handler.FileStatus = handler.Services.POST.GetPostedFiles();
            if (handler.FileStatus != null)
            {
                ...
                handler.Services.POST.CreateThumbnail(file);

                // Create a base64 encoded data url for the thumbnail we can return in Json
                handler.Services.Core.CreateThumbnailDataUrls();
